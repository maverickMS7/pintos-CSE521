...
syscall_handler (struct intr_frame *f)
{
  ...
  check_buffer((void *) f->esp, sizeof(int));
...
//        }
    ...
    case SYS_CREATE: {
      check_args(args, 2);
      check_buffer((void *)args[0], sizeof(char *));
...
//    if (file_get_inode(file) == file_inodes[i])
//      file_deny_write(temp_f);
//  }
...
/* Definition from Linux man page: size_t write(int fd, const void *buf, size_t count); */
...
void check_addr (const void *vaddr){
  ...
  void *val = pagedir_get_page(t->pagedir, vaddr);
  if (val == NULL) {
    exit(-1);
    NOT_REACHED();
  }
